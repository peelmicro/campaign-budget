...
export class CampaignDetailComponent implements OnInit {
  ...
  readonly campaign = computed(() =>
    this.service.campaigns().find((c) => c.id === this.id),
  );

  readonly totalImpressions = computed(() =>
    (this.campaign()?.channels || []).reduce(
      (sum, ch) => sum + ch.estimatedImpressions,
      0,
    ),
  );
  ...
  readonly totalReach = computed(() =>
    (this.campaign()?.channels || []).reduce(
      (sum, ch) => sum + ch.estimatedReach,
      0,
    ),
  );

  ngOnInit(): void {
    this.service.getCampaign(this.id);
    const c = this.campaign();
    if (c) {
      this.selectedGoal = c.goal;
    }
  }
  ...
  onRedistribute(): void {
    ...
    if (!c || this.selectedGoal === c.goal) {
      this.service.redistribute(this.id);
    } else {
      this.service.updateCampaign(this.id, { goal: this.selectedGoal }, () => {
        // Goal changed — distribution recalculated by the API
      });
    }
  }
  ...
  onDelete(): void {
    if (confirm('Are you sure you want to delete this campaign?')) {
      this.service.deleteCampaign(this.id, () => {
