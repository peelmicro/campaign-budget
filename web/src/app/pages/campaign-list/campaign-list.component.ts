...
@Component({
  ...
  template: `
    <div class="max-w-6xl mx-auto p-6">
      ...
      @if (service.error()) {
        <p class="text-red-600 bg-red-50 p-3 rounded">{{ service.error() }}</p>
      }

      @if (!service.loading() && campaigns().length === 0) {
            ...
            <tbody>
              @for (campaign of campaigns(); track campaign.id) {
                <tr>
                  ...
                  <td class="px-4 py-3 text-gray-700">{{ campaign.clientName }}</td>
