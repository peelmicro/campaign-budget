...
![Demo](docs/demo.gif)
*End-to-end flow: campaign list → create → auto-distribution → re-distribute across REACH / ENGAGEMENT / BALANCED goals → schedule modal.*
...
# Option 1: Docker (recommended) — no Node.js install needed
docker-compose up
...
*All three services (`mysql`, `api`, `web`) running from a single `docker-compose up` — the API seeds 3 currencies, 3 channels, and 3 demo campaigns on startup.*

The application starts with 3 demo campaigns pre-seeded (REACH, ENGAGEMENT, BALANCED goals) so you can see distribution results immediately.

![Campaign list](docs/home.png)
*Landing page at http://localhost:4200 — 3 seeded campaigns with color-coded goal badges (green REACH, purple ENGAGEMENT, blue BALANCED) and currency-formatted budgets.*
...
![REACH distribution](docs/campaign-detail-reach.png)
*Campaign detail — REACH goal. Social dominates the allocation (~66.7%) because its low CPM converts into the most impressions per dollar.*
...
![ENGAGEMENT distribution](docs/campaign-detail-engagement.png)
*Campaign detail — ENGAGEMENT goal. Video takes the largest slice (~53.6%) because its high CPM correlates with higher-quality interactions.*

### BALANCED goal — equal distribution

![BALANCED distribution](docs/campaign-detail-balanced.png)
*Campaign detail — BALANCED goal. Equal 33.3% allocation across all three channels regardless of CPM.*

### Re-distribute on demand

![Re-distribute dropdown](docs/redistribute.png)
*Selecting a different goal and clicking **Redistribute** updates the campaign via `PATCH /api/campaigns/:id` and recalculates every channel's allocated budget, impressions, and reach in place.*

### Per-channel schedule

![Schedule modal](docs/schedule-modal.png)
*Clicking **View dates** on any channel row opens the 30-day schedule generated by the distribution service — each channel runs for the full campaign duration, with one row per day.*

### Create a new campaign

![Campaign form](docs/campaign-form.png)
*Creating a campaign auto-triggers budget distribution — campaign managers get immediate results without a separate distribution step.*
...
`.http` files for VS Code REST Client live inside each module folder (e.g., [apps/api/src/campaign/campaign.http](apps/api/src/campaign/campaign.http)).

![REST Client](docs/rest-client.png)
*Executing `POST /api/campaigns` from VS Code's REST Client — the response includes the auto-calculated channel allocations (budget, impressions, reach, schedule) for all three channels.*
...
*`pnpm api:test` — 53 tests across 7 files, all passing (Vitest).*
...
*`pnpm web:test` — 13 tests across 4 files, all passing (Vitest + Angular 21 test runner).*
...
│       └── src/
│           ├── main.ts                      # ConsoleLogger({ json: true })
│           ├── app/
│           │   └── app.module.ts            # TypeORM.forRoot + module imports
│           ├── currency/                    # Currency entity, service, controller, tests
│           ├── channel/                     # Channel entity + EngagementRank enum
│           ├── campaign/                    # Campaign + CampaignChannel entities, DTOs
...
- `clientName` and `managerName` are strings. In production, these should be separate entities with relationships.
- Currency conversion is not implemented. All channels and campaigns must use the same currency. In production, we'd integrate with exchange rate APIs.
